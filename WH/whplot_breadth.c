            ...
            for(q = adata->q_min; q < adata->q_max; q += adata->q_step)
            {
                for(Ch00 = adata->Ch00_min; Ch00 < adata->Ch00_max; Ch00 += adata->Ch00_step)
                {
                    ...
                    for(j = 0; j < cdata->npeaks; j++)
                    {
                        ...
                        {
                            fit_data->x[k] = 2. * sin(angles->theta_grad[j][i]*radian) / adata->lambda * sqrt(Chkl(Ch00, q, cdata->indices[j]));
                            fit_data->y[k] = widths->breadth[j][i] - delta * cdata->warrenc[j];
                            fit_data->y_err[k] = widths->breadth_err[j][i];
                            k++;
                        }
                    }
        ...
        for(delta = adata->delta_min; delta < adata->delta_max; delta += adata->delta_step)
        {
            for(q = adata->q_min; q < adata->q_max; q += adata->q_step)
            {
                for(Ch00 = adata->Ch00_min; Ch00 < adata->Ch00_max; Ch00 += adata->Ch00_step)
                {
                    ...
                    for(j = 0; j < cdata->npeaks; j++)
                    {
                        if(widths->breadth_corr[j][i] > 0)
                        {
                            fit_data->x[k] = 2. * sin(angles->theta_grad[j][i]*radian) / adata->lambda * sqrt(Chkl(Ch00, q, cdata->indices[j]));
                            fit_data->y[k] = widths->breadth_corr[j][i] - delta * cdata->warrenc[j];
                            fit_data->y_err[k] = widths->breadth_corr_err[j][i];
                            k++;
                        }
                    }
                    gsl_fit_wlinear(fit_data->x, 1, fit_data->y_err, 1, fit_data->y, 1, k, &fit_data->h,  &fit_data->m,
...
void williamson_hall_plot_breadth_3(int nlines, aux_data * adata, crystal_data * cdata, shape_params * widths, angles_grad * angles, linear_fit * fit_data, best_values * out_values)
{
    int i, j, k;
    double radian = M_PI / 360., delta, q, Ch00;
    for(i = 0; i < nlines; i++)
                ...
                for(Ch00 = adata->Ch00_min; Ch00 < adata->Ch00_max; Ch00 += adata->Ch00_step)
                {
                    ...
                    for(j = 0; j < cdata->npeaks; j++)
                    {
                        if(widths->breadth[j][i] > 0)
                        {
                            ...
                            fit_data->y[k] = pow(widths->breadth[j][i] - delta * cdata->warrenc[j], 2.0);
                            fit_data->y_err[k] = 2 * (widths->breadth[j][i] - delta * cdata->warrenc[j]) * widths->breadth_err[j][i];
                            k++;
                        }
                    }
                    gsl_fit_wlinear(fit_data->x, 1, fit_data->y_err, 1, fit_data->y, 1, k, &fit_data->h,  &fit_data->m,
...
void williamson_hall_plot_breadth_4(int nlines, aux_data * adata, crystal_data * cdata, shape_params * widths, angles_grad * angles, linear_fit * fit_data, best_values * out_values)
{
    int i, j, k;
    double radian = M_PI / 360., delta, q, Ch00;
    for(i = 0; i < nlines; i++)
            ...
            for(q = adata->q_min; q < adata->q_max; q += adata->q_step)
            {
                for(Ch00 = adata->Ch00_min; Ch00 < adata->Ch00_max; Ch00 += adata->Ch00_step)
                {
                    ...
                    for(j = 0; j < cdata->npeaks; j++)
                    {
                        ...
                        {
                            fit_data->x[k] = pow(2. * sin(angles->theta_grad[j][i]*radian) / adata->lambda, 2.0) * Chkl(Ch00, q, cdata->indices[j]);
                            fit_data->y[k] = pow(widths->breadth_corr[j][i] - delta * cdata->warrenc[j], 2.0);
                            fit_data->y_err[k] = 2 * (widths->breadth_corr[j][i] - delta * cdata->warrenc[j]) * widths->breadth_corr_err[j][i];
                            k++;
                        }
                    }
                    gsl_fit_wlinear(fit_data->x, 1, fit_data->y_err, 1, fit_data->y, 1, k, &fit_data->h,  &fit_data->m,
