                ...
                for(Ch00 = adata->Ch00_min; Ch00 < adata->Ch00_max; Ch00 += adata->Ch00_step)
                {
                    ...
                    for(j = 0; j < cdata->npeaks; j++)
                    {
                        if(widths->breadth[j][i] > 0)
                        {
                            ...
                            fit_data->y_err[k] = widths->breadth_err[j][i] * cos(angles->theta_grad[j][i] * radian) / adata->lambda;
                            k++;
                ...
                for(Ch00 = adata->Ch00_min; Ch00 < adata->Ch00_max; Ch00 += adata->Ch00_step)
                {
                    ...
                    for(j = 0; j < cdata->npeaks; j++)
                    {
                        if(widths->breadth_corr[j][i] > 0)
                        {
                            ...
                            fit_data->y_err[k] = widths->breadth_corr_err[j][i] * cos(angles->theta_grad[j][i] * radian) / adata->lambda;
                            k++;
                        }
                    }
                    //print_xy(fit_data->x, fit_data->y, fit_data->y_err, k);
