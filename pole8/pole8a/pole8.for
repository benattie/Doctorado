...
c
c ***********************************************************************
c ***   program pole8   ***   version 08/MAR/07   ***   Carlos Tome   ***
c ***********************************************************************
...
c ***                                                                 ***
c *** Creates an output file POLE7.OUT containing the information     ***
c *** about the pole figure for plotting with PLOTPF7.FOR.            ***
...
c ***     * handles texture files with more than one texture.         ***
...
c ***********************************************************************
      ...
      character*80 textfile(9),textfilex(9),filecrys,prosa
      character    nfile*12,nomen*1
      character*10 time,date,zone
      integer      dt(8),miller(4,8)
      integer*2    default
...
c *** reads-in texture files & splits files that contain multiple textures
      ...
      do i=1,ntextfilesx
        ...
        do multi=1,20       ! eventually sets limit to 9 textures total
     ...
     #                '' TEXTURE FILE'')') NGRAIN
          ...
          textfile(ntextfiles)='text_'//char(48+ntextfiles)//'.dat'
   ...
   77   continue
      ...
      if(ndiftexts.gt.9) then
        write(*,*) ' TOTAL # OF TEXTURES CANNOT EXCEED 9'
        stop
      endif
...
c *** reads Miller indices of poles (or sample axes) to be plot
      ...
      read(ur1,'(a)') prosa
      read(ur1,*)     ndifpoles
      if(ndifpoles.gt.8) then
        write(*,*) ' TOTAL NUMBER OF POLES CANNOT EXCEED 8'
        stop
      endif
      read(ur1,'(a)') prosa
      do jpol=1,ndifpoles
...
c **********************************************************************
c *** calculates radious (in inches) of PF or IPF consistent with the
c     number of different poles and texture files.
c     For cubic IPF calculates a factor to expand the IPF to radious size.
...
ccc   nrows=max(ndifpoles,ndiftexts)    ! temporary inversion 02/22/07
ccc   ncols=min(ndifpoles,ndiftexts)
...
c *** Write heading with information about this run in file POLE8.OUT
     ...
     #                   dt(1),dt(2),dt(3),dt(5),dt(6)
      write(uw1,'(a)') (textfilex(i),i=1,ntextfilesx)
...
ccc     pfrot=0.                             ! temporary....
ccc     if(ndiftex.eq.1) pfrot=90.*pi/180.   ! temporary....
...
cc ---> temporary inversion to stretch vertically (02/22/07)
cc      if(ndifpoles.ge.ndiftexts) then
cc        nyy=ndifpol
cc        nxx=ndiftex
cc      else if(ndifpoles.lt.ndiftexts) then
cc        nyy=ndiftex
cc        nxx=ndifpol
cc      endif
...
C     WRITE(*,'(''TOTAL WEIGHT = '',F10.6)') TOTWGT
