 ...
 * 2.1 	      26/04/06	Keyword DETECTOR fixed
 * 2.0 	      07/11/05	Added Writemar345Header for use with buffered I/O
 * 1.11	      15/11/04	Make sure that h345.date does not exceed 26 chars
 *			h345.remark is < 56, h345.filter < 26, h345.source < 32
 *
 * 1.10	      25/10/03	DATE and REMARK would return zero strings. Fixed
 *			by copying token[i]
...
extern void WrongType(int, char*, char*);
extern void RemoveBlanks(char*);
...
int Writemar345Header(FILE*, MAR345_HEADER);
static int PutHeader(int, FILE*, MAR345_HEADER);
    ...
    while (fgets(buf, 64, fp) != NULL) {
        ...
        for (i = 0; i < ntok; i++) {
            ...
            for (j = 0; j < strlen(str); j++)
                if (isalnum(str[j]) && !strstr(key, "PROG") && !strstr(key, "DATE") &&
                    !strstr(key, "REMA") && !strstr(key, "DETE"))
                    str[j] = toupper(str[j]);
            strcpy(token[i], str);
        ...
        /* Keyword: DATE */
            ...
            h.date[0] = '\0';
            for (i = 0; i < ntok; i++) {
                /* Version 1.11: check max. strlen */
                if ((strlen(h.date) + strlen(token[i]) + 1) < 26) {
                    strcat(h.date, token[i]);
                    if (i < (ntok - 1))
                        strcat(h.date, " ");
                }
            }
        ...
        /* Keyword: REMARK */
            ...
            for (i = 0; i < ntok; i++) {
                /* Version 1.11: check max. strlen */
                if ((strlen(h.remark) + strlen(token[i]) + 1) < 56) {
                    strcat(h.remark, token[i]);
                    if (i < (ntok - 1))
                        strcat(h.remark, " ");
                }
            }
        ...
        /* Keyword: MONOCHROMATOR */
            ...
            for (i = 0; i < ntok; i++) {
                if (strstr(token[i], "POLA")) {
                    ...
                    /* Version 1.11: check max. strlen */
                    if ((strlen(h.filter) + strlen(token[i]) + 1) < 32) {
                        strcat(h.filter, token[i]);
                        if ((strlen(h.filter) + strlen(token[i]) + 1) < 32 && i < (ntok - 1))
                            strcat(h.filter, " ");
                    }
                }
            }
        ...
        /* Keyword: GENERATOR */
            ...
            for (i = 0; i < ntok; i++) {
                if (strstr(token[i], "MA")) {
                    ...
                    if ((strlen(h.source) + strlen(token[i]) + 1) < 32) {
                        strcat(h.source, token[i]);
                        if ((strlen(h.source) + strlen(token[i]) + 1) < 32 && i < (ntok - 1))
                            strcat(h.source, " ");
                    }
                }
            }
 ...
 ******************************************************************/
int Putmar345Header(int fd, MAR345_HEADER h)
{
    if (fd < 0)
        return (0);
    return (PutHeader(fd, NULL, h));
}

/******************************************************************
 * Function: Writemar345Header
 ******************************************************************/
int Writemar345Header(FILE* fp, MAR345_HEADER h)
{
    if (fp == NULL)
        return (0);
    return (PutHeader(-1, fp, h));
}

/******************************************************************
 * Function: PutHeader
 ******************************************************************/
static int PutHeader(int fd, FILE* fp, MAR345_HEADER h)
{
    int i, k = 0;
    int io;
    int head[32];
    ...
    if (fp != NULL) {
        io = fwrite(head, sizeof(int), 32, fp) * sizeof(int);
    } else
        io = write(fd, head, 32 * sizeof(int));

    sprintf(str, "mar research");
    for (i = strlen(str); i < 63; i++)
        str[i] = ' ';
    str[63] = '\n';
    if (fp != NULL) {
        io += fwrite(str, sizeof(char), 64, fp);
        k++;
    } else {
        io += write(fd, str, 64);
        k++;
    ...
    sprintf(str, "GAPS           %d %d %d %d %d %d %d %d\n", h.gap[0], h.gap[1], h.gap[2], h.gap[3],
            h.gap[4], h.gap[5], h.gap[6], h.gap[7]);
