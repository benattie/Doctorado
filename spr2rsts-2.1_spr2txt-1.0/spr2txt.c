...
main()
{
 ...
 for(Z=1;Z<=NrSample;Z++) // FOR-routine: whole routines
 {
    ...
    {
	char csv_file[50]; //string en el que voy a guardar el nombre del archvo csv que sale de spr2csv.py
	char rsts_file[50]; //string en el que voy a guardar el nombre del archvo rsts que sale de spr2csv.py
	...
	strcpy(csv_file, marfile); //hasta aca marfile tiene la misma estructura que el csv
	strcat(csv_file, "csv");

	strcpy(rsts_file, marfile); //hasta aca marfile tiene la misma estructura que el rsts
	strcat(rsts_file, "rsts");
...
/////////////////////////////////////////////////EXPERIMENTAL////////////////////////////////////////////////////       
	...
	sprintf(py_cmd, "python spr2csv.py %s \"  \" %s %s %d %s", marfile, csv_file, rsts_file, numrings, filename1);
	system(py_cmd);
