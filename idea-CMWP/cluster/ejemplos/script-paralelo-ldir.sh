...
echo " "
cp -v $SGE_O_WORKDIR/{INCAR,POSCAR,POTCAR,KPOINTS}         $TMP_WORK_DIR/
...
$IMPI_HOME/mpirun -machinefile $TMPDIR/machines -l -genv I_MPI_DEVICE sock -n $NSLOTS /home/fbusnengo.ifir/VASP/VASP5.3/bin/vasp5.3.3-par

# -------- SECTION clean up files ---------------------------------------------
echo " "
echo "Cleaning up files ... removing unnecessary files ..."
echo " "
rm -vf CHG EIGENVAL* IBZKPT* PCDAT* PROCAR* \
       DOSCAR* ELFCAR* LOCPOT* OUTPAR* PROOUT* \
       CHGCAR* TMPCAR* WAVECAR* POTCAR 
