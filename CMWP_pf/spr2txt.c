...
int main()
{
 ...
 double pixel, dist;
 double ***fit_ttheta = r3_tensor_double_alloc(40, 500, 10);
 ...
 for(Z = 1; Z <= NrSample; Z++) // FOR-routine: whole routines
 {
    ...
    //LEER LOS INDICES HKL DE CADA PICO
    ...
    do //Iteracion sobre todos los spr  
    {
        ...
        for(y = 1; y <= gamma; y++) //itero sobre todos los difractogramas (360) (recorro el anillo de Debye)
        {
            ...
            if((y % del_gam) == 0)
            {
                ...
                peak_data difra = {numrings, hkl, bg_size, k, star_d, y, del_gam, data1, bg_seed, fit_ttheta, fit_inten, &shapes, &fit_errors};
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
 ...
 fclose(fp);
 free_int_matrix(hkl, 15);
 free_r3_tensor_double(fit_ttheta, 40, 500);
